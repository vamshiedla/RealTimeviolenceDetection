...
function App() {
  ...
  const captureSnapshot = () => {
    if (videoRef.current && canvasRef.current) {
      ...
      if (context) {
        // Resize to a very small snapshot to fit EmailJS 50KB limit and email display
        const maxWidth = 100; // Very small width
        const maxHeight = 75; // Very small height
        ...
        return canvasRef.current.toDataURL('image/jpeg', 0.5); // Lower quality further
      }
    }
    return null;
  };

  const uploadSnapshot = async (dataUrl: string) => {
    const apiKey = '145faf79af8c9ec07a639dbd2916536c'; // your imgBB API key
  ...
  const sendEmailAlert = async (snapshot: string | null, violenceLevel: number) => {
    try {
      let snapshotUrl = null;
      if (snapshot) {
        snapshotUrl = await uploadSnapshot(snapshot);
      }

      const templateParams = {
        to_email: 'evamshi.dev@gmail.com',
        location: location ? `${location.latitude}, ${location.longitude}` : 'Location not available',
        timestamp: new Date().toLocaleString(),
        snapshot: snapshot || 'Snapshot not available',
        snapshot_url: snapshotUrl || 'Snapshot not available',
        violence_level: Math.round(violenceLevel)
      };
  ...
  const triggerAlert = async (violenceLevel: number) => {
    ...
    await sendEmailAlert(snapshot, violenceLevel);
    ...
    if (violenceProbability > 95) {
      if (!alertTimeoutRef.current) {
        alertTimeoutRef.current = setTimeout(() => {
          triggerAlert(violenceProbability);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-gray-800 rounded-lg p-6">
              <h1 className="text-2xl font-bold mb-4">Guardian Eye: Real‑Time Surveillance Monitoring with Automated Alerts Using Deep Learning</h1>
